...
[arabic]
. *Tier 1 sidecar isolation*: Tier 1 operations NEVER write to the
target database.
. *Provenance append-only*: Hash chain records are immutable once
written.
. *Octad completeness*: All 8 dimensions accounted for (data, metadata,
provenance, lineage, constraints, access-control, temporal, simulation).
. *Drift completeness*: All 8 drift categories covered (structural,
semantic, temporal, statistical, referential, provenance, spatial,
embedding).
. *C-ABI compatibility*: All FFI types match between Idris2 declarations
and Zig implementations.

=== Interception Strategies

[width="100%",cols="19%,62%,19%",options="header",]
|===
|Backend |Method |Type
...
[width="100%",cols="27%,15%,13%,45%",options="header",]
|===
...
* *VeriSimDB* (nextgen-databases): Source of the octad model.
VeriSimiser is a gateway to full VeriSimDB adoption.
* *TypedQLiser*: Compile-time query type checking. Works alongside
VeriSimiser for formally verified queries.
* *SqueakWell*: Database recovery via cross-modal constraint
propagation. Uses VeriSimiser’s drift detection.
